...
export default function AdminUsersPage() {
  ...
  const handleDelete = async (id: string) => {
    const isConfirmed = await confirm("Supprimer l'utilisateur", "Êtes-vous sûr de vouloir supprimer cet utilisateur ?");
  ...
  const openModal = (user?: User) => {
    if (user) {
      ...
      setEditingUser(null);
      setFormData({
        firstName: '', lastName: '', email: '', phone: '', role: 'USER', isActive: true,
      });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        ...
        await (apiClient as any).createUser(formData).catch(() => {
            throw new Error("Creation endpoint might need password. Using update instead.");
        });
      ...
      if (!editingUser) {
          toast.info("User creation requires a password in current backend. Please use the Registration page.");
      } else {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {isLoading ? (
          ...
          <AdminGridCard
            ...
            glowClass={user.role === 'ADMIN' ? 'bg-gold/10 group-hover:bg-gold/20' : 'bg-blue-500/5 group-hover:bg-blue-500/10'}
            icon={user.role === 'ADMIN' ? <Shield className="w-6 h-6" /> : <UserIcon className="w-6 h-6" />}
          >
            ...
            <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/5">
              ...
              <div className="flex flex-col items-end">
                <span className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-1">Joined</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" name="firstName" value={formData.firstName} onChange={handleInputChange} required className={inputClass} placeholder="John" />
            </div>

            <div>
              <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-widest">Last Name</label>
              <input type="text" name="lastName" value={formData.lastName} onChange={handleInputChange} required className={inputClass} placeholder="Doe" />
            </div>
            ...
            <div>
              ...
              <input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} className={inputClass} placeholder="+1 234 567 890" />
            </div>

            <div>
              <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-widest">Role</label>
              <select name="role" value={formData.role} onChange={handleInputChange} className={`${inputClass} appearance-none`}>
            ...
            <div className="md:col-span-2 border-t border-white/5 pt-6 mt-2">
              <label className="flex items-center gap-3 p-4 bg-white/5 border border-white/10 rounded-xl cursor-pointer hover:bg-white/10 transition-colors">
                ...
                <div>
                  <span className="block text-sm font-bold text-white">Active Account</span>
                  <span className="text-xs text-gray-500">Uncheck to suspend this user's access</span>
                </div>
              </label>
            </div>
          </div>
